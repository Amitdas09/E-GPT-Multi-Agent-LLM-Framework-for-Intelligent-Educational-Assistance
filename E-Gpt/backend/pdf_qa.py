...
class DocumentQASystem:
    ...
    def _is_scanned_pdf(self, page) -> bool:
        """Check if a PDF page is scanned (image-based) rather than text-based."""
        # Get text blocks from the page
        text_dict = page.get_text("dict")
        
        # If there are no text blocks or very few characters, it's likely scanned
        text_blocks = [block for block in text_dict.get("blocks", []) if block.get("lines")]
        total_chars = sum(len(block.get("lines", [])) for block in text_blocks) if text_blocks else 0
        
        # If there are very few text characters, consider it scanned
        if total_chars < 10:  # Less than 10 characters likely means scanned
            return True
        
        # Additional check: if the page has images but almost no text, it's scanned
        images = page.get_images()
        if images and total_chars < 50:  # Has images and very little text
            return True
        ...
        try:
            ...
            if file_ext == '.pdf':
                ...
                for page_num in range(len(doc)):
                    page = doc[page_num]
                    # Check if the page is scanned (image-based)
                    if self._is_scanned_pdf(page):
                        # Use OCR for scanned pages
                        logger.info("Detected scanned page, using OCR")
    ...
    def create_chunks(self, text: str) -> List[str]:
        ...
        try:
            ...
            chunks = text_splitter.split_text(text.lower())
    ...
    def create_vector_store(self, chunks: List[str], is_new_index: bool = True):
        ...
        try:
            if is_new_index:
                ...
                # Check if vector_store has add_texts method before calling
    ...
    def setup_qa_chain(self):
        ...
        try:
            ...
            # Add None check for vector_store
    ...
    def load_vector_store(self) -> bool:
        ...
        try:
            ...
            if index_path.exists():
                ...
                self.vector_store = FAISS.load_local(str(self.index_dir), self.embeddings, allow_dangerous_deserialization=True)
                self.setup_qa_chain()
    ...
    def correct_spelling(self, text: str) -> str:
        """Spell-correct the user query using pyspellchecker."""
        spell = SpellChecker()
        corrected = [spell.correction(word) or word for word in text.split()]
    ...
    def answer_question(self, question: str) -> dict:
        ...
        try:
            ...
            # Correct spelling before querying
            corrected_question = self.correct_spelling(question)
            logger.info(f"Answering question (spell-corrected): {corrected_question}")
            result = self.qa_chain.invoke({"query": corrected_question.lower()})
